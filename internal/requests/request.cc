...
#include <stdexcept>
#include <string>
// producer produces raww bytes recevied from socket
// parser will conver that into an HTTP request
/*
HTTP_requestLine parserHTTP_requestLine(const std::string &rawData) {
  HTTP_requestLine clientRequest;
  std::string temp_requestLine{}, temp_restofData{};
  std::vector<std::string> temp_requestLine_split;

  size_t pos = rawData.find(CRLF);

  temp_requestLine =
      rawData.substr(0, pos); // everything before \r\n --> 2 bytes
  temp_restofData = rawData.substr(pos + 2); // rest of data
  ...
  if (temp_requestLine_split.size() != 3)
    throw std::runtime_error("Request line must contain exactly 3 parts");
  //
  ...
  if (temp_requestLine_split[1][0] != '/') {
    throw std::runtime_error("Invalid target request");
  }
  ...
  if (temp_requestLine_split[2] != "HTTP/1.1") {
    ...
    clientRequest.requestLine.HTTP_method   = temp_requestLine_split[0];
  clientRequest.requestLine.targetRequest = temp_requestLine_split[1];
  clientRequest.requestLine.HTTP_version  = temp_requestLine_split[2].substr(5);
// after /
